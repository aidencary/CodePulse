...
| NFR-SEC-007 | Rate Limiting | `/analyze` endpoint rate-limited to 10 requests/minute per bearer token via `slowapi`; falls back to remote address for unauthenticated traffic; returns 429 Too Many Requests when exceeded | `app/limiter.py`, `app/main.py` |
